...
*   **Reporting Bias Disclaimer:** The dataset documentation states "the tip for the trip. Cash tips generally will not be recorded.Data profiling reveals a massive documentation variance across payment types. While electronic methods like `Mobile` (94.03%) and `Credit Card` (93.80%) near-perfectly capture tips, `Cash` transactions only record tips **0.32%** of the time. This confirms a structural tracking limitation in the Chicago Open Data portal cash tips are simply not reported by drivers, causing an artificial under ranking for cash heavy drivers in Q1.
